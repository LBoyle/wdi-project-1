var game = game || {
  ...
  createBoard() {
    ...
    const boardSize = (this.keys.length*50)+(this.keys.length*2); // allows for adaptive size, like 10x10 grid etc, not recommended, tried to get rid of magic numbers, the init squares are no longer in the center
  ...
  taskDist() { // this move switches game modes as chosen py the user
    if ($('.N').length > 0) { // if there are free playable squares,
      ...
      if (this.gameMode === 'pvp') {
        ...
        this.playerController(); // call the function that handles players only
        ...
        this.compController(); // this function sets the computer running
      }
  ...
  getPlayer() {return [(this.count === 0 || this.count % 2 === 0) ? 'B' : 'W', (this.count === 0 || this.count % 2 === 0) ? 'W' : 'B'];}, // find out whose turn it is, could probably refactor to use this more
  playerController() { // not for computers
    ...
    if (legals.length > 0) { // returns boolean to find if player has legal moves
      ...
      this.count++; // skip this players turn if there are no legal moves, not tested because it's hard to pull off
      this.taskDist(); // call the parent function again to wait for input
    }
  },
  ...
  hasWinner(winner) {$('.instr').text(`Game over, ${winner}`);}, // update the DOM with winner
  ...
  checkInput(e, player, enemy) { // this function differentiates between player and computer
    ...
    if (e) { // if e has been delivered by a click event, it was the user, if e is undefined it's the com
      if (!$(e.target).hasClass('clicked')) { // check if it's an N square
        ...
        this.count++; // initially increment
        const chipsToFlip = this.getChips(row, col, player, this.boardModel, this.keys); // check the user choice
      ...
      this.count++; // always increment the count because com never misclicks
      const cpResponse = this.computerPlay(player, this.boardModel, this.keys); // this is whichever square the computer has chosen
      if (cpResponse.length > 0) {
        ...
        const chipsToFlip = this.getChips(row, col, player, this.boardModel, this.keys); // check com choice the same as for players
  ...
  play(player, enemy, row, col, chipsToFlip) {
  ...
  numTiles() { // this retrieves the numer of tiles at any time for each player
    ...
    for (var i=0; i<this.keys.length; i++) {
      for (var j=0; j<this.boardModel[this.keys[i]].length; j++) {
        if (this.boardModel[this.keys[i]][j] === 'B') blks++;
  ...
  doFlip(enemy, player, chipsToFlip) { // chipsToFlip === getChips()
    for (let i=0; i<chipsToFlip.length; i++) {
      if (!$(chipsToFlip[i]).hasClass('N')) { // just to be absolutely sure
        ...
        this.boardModel[thisChip[1]][thisChip[2]] = player; // update said boardModel
  ...
  getChips(row, col, player, board, keys) { // this one written by Alex, I watched intently
    ...
    const possiblePlanes = directions.map(direction => { // array.map() returns a list
      ...
      for (let j = 0; j < keys.length-1; j++) { // limit to 6 pieces takeable on any one axis, will stop scanning if there are no takeable pieces adjacent
        ...
        if (this.checkLength(plane, newCol, newRow, keys)) { // I wrote this check to prevent some illegal moves
  ...
  checkLength(plane, col, row, keys) {return plane.length >= 6 && (col === keys.length-1 || col === 0 || row === 'a' || row === keys[keys.length-1]);}, // I added this one to prevent some illegal moves
  invalidMove(row, col, square, keys) {return this.validRow(row) || this.validColumn(col, keys) || this.emptySquare(square);}, // mother function for the three checks below
  validRow(row) {return typeof row === 'undefined';}, // conditions to break
  validColumn(col, keys) {return col > keys.length-1 || col < 0;}, // conditions to break
  emptySquare(square) {return square === 'N';}, // conditions to break
  ...
  anyLegalMoves(player, board, keys, isPlayer) {
    const possible = this.getChoices(player, board, keys, isPlayer); // isPlayer is a magic number, or boolean
